...
import { LayoutDashboard, Users, ShoppingBag, Settings, LogOut, Menu, X, ChevronRight } from 'lucide-react'
    ...
    <div className="flex h-screen bg-[#f8f7ff] overflow-hidden">
      ...
      <aside className={`${sidebarOpen ? 'w-56' : 'w-16'} transition-all duration-300 bg-white border-r border-gray-100 flex flex-col shadow-sm shrink-0`}>
        <div className="h-16 flex items-center px-4 border-b border-gray-100 gap-3">
          ...
          {sidebarOpen && <span className="font-semibold text-gray-800 text-sm">Veloura</span>}
        </div>
            ...
            return (
              <Link key={href} href={href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                  ${active ? 'bg-pink-50 text-pink-600 font-medium' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
              >
                <Icon size={18} className="shrink-0" />
                {sidebarOpen && <span>{label}</span>}
                {sidebarOpen && active && <ChevronRight size={14} className="ml-auto text-pink-400" />}
              </Link>
            )
          })}
        </nav>
