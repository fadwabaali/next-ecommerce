...
export default function OrdersPage() {
  ...
  const clientFilter = searchParams.get('client') // from clients page "View orders" button
  ...
  async function fetchOrders() {
    ...
    const { data } = await supabase
      .from('orders')
      .select('id, status, total, store, notes, items, created_at, clients(id, name, phone, city)')
      .order('created_at', { ascending: sortDir === 'asc' })

    let result = (data as Order[]) || []

    // If coming from clients page with ?client=id
    if (clientFilter) {
      result = result.filter(o => o.clients?.id === clientFilter)
    }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-lg text-xs text-gray-500 hover:border-pink-300 transition"
        >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-400">Items</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-400">Total</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-400">Status</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-400">Store</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center text-gray-400 text-xs">Loading...</td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center text-gray-400 text-xs">No orders found</td>
                </tr>
              ) : filtered.map(order => (
                <tr key={order.id} className="hover:bg-gray-50 transition">
                    ...
                    {editingTotal === order.id ? (
                      ...
                      <button
                        onClick={() => {
                          ...
                          setTotalValue(order.total?.toString() ?? '')
                        }}
                  ...
                  <td className="px-4 py-3.5 text-gray-400 text-xs">
                    {order.store ?? <span className="italic text-gray-300">—</span>}
                  </td>
                      ...
                      {confirmDelete === order.id ? (
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded-lg text-xs transition disabled:opacity-50"
                          >
                            {deletingId === order.id ? '...' : 'Yes'}
                          </button>
                          <button
                            onClick={() => setConfirmDelete(null)}
                            className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg text-xs transition"
                          >
                            No
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          className="p-1.5 bg-red-50 hover:bg-red-100 text-red-400 rounded-lg transition"
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
